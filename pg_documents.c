...
pg_documents_all_tables(struct pg_documents_context *pgdc)
{
	...
	elog(DEBUG1, "%ld document tables found", SPI_processed);
...
pg_documents_create_attachment(struct pg_documents_context *pgdc)
{
	...
	values[4] = (Datum)DatumGetByteaP(DirectFunctionCall1(byteain,
...
pg_documents_get_attachment(struct pg_documents_context *pgdc)
{
	...
	elog(DEBUG1, "%ld attachments found for %s: %s", SPI_processed,
...
pg_documents_get_document(struct pg_documents_context *pgdc)
{
	...
	if (SPI_processed == 1)
	{
		...
		{
			int i;
			json_object_s *jsona = json_object_new_object();
			bool isnull;

			for (i = 0; i < SPI_processed; i++)
			{
				json_object_s *jsonb = json_object_new_object();
				json_object_s *jsoni;
				json_object_s *jsons;
...
pg_documents_main(Datum main_arg)
{
	...
	BackgroundWorkerInitializeConnection(pg_documents_database, NULL, 0);
#else
		...
		else
		{
			if (FD_ISSET(socket_listener, &socks))
			{
				...
				if (sockfd == -1)
				{
					l_errno = errno;
					if (errno != EAGAIN)
						elog(WARNING, "accept() error: %d",l_errno);
					continue;
				}
...
_PG_init(void)
{
	...
	sprintf(worker.bgw_library_name, "pg_documents");
